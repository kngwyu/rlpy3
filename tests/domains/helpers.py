...
def check_seed_vis(make_exp_fun):
    ...
    expNoVis.run(
        visualize_steps=False, visualize_learning=False, visualize_performance=0
    )

    # [[initialize and run experiment with visual]]
    expVis1 = make_exp_fun(exp_id=1)
    expVis1.config_logging = False
    expVis1.run(visualize_steps=False, visualize_learning=True, visualize_performance=0)
    plt.close("all")

    expVis2 = make_exp_fun(exp_id=1)
    expVis2.config_logging = False
    expVis2.run(visualize_steps=False, visualize_learning=True, visualize_performance=1)
    plt.close("all")
