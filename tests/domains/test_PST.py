...
def test_errs():
    ...
    sens = np.array([1, 1])
    assert np.all(
        domain.properties2StateVec(locs, fuel, act, sens) == [1, 2, 9, 3, 1, 0, 1, 1]
    )
...
def test_transitions():
    ...
    r, ns, t, possA = domain.step(a)
    assert np.array_equiv(
        ns, domain.properties2StateVec(locs + 1, fuel - 2, act, np.array([0, 0]))
    )
    ...
    assert np.array_equiv(
        ...
        domain.properties2StateVec(
            locs + [0, 1], fuel - 3, np.array([0, 0]), np.array([0, 0])
        ),
    )
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(locs, fuel - 4, np.array([0, 0]), np.array([0, 0])),
    )
    ...
    assert np.array_equiv(
        ns, domain.properties2StateVec(locs, fuel, np.array([0, 0]), np.array([0, 0]))
    )
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(
            locs - 1, fuel - 1, np.array([0, 0]), np.array([0, 0])
        ),
    )
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(
            locs - 1, fuel - 1, np.array([1, 1]), np.array([1, 1])
        ),
    )
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(locs, fuel - 2, np.array([1, 1]), np.array([1, 1])),
    )
    actionVec = np.array([UAVAction.ADVANCE, UAVAction.ADVANCE])
    a = vec2id(actionVec, actionLimits)
    r, ns, t, possA = domain.step(a)
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(
            locs + 2, fuel - 2, np.array([1, 1]), np.array([1, 1])
        ),
    )
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(locs, fuel - 3, np.array([1, 1]), np.array([1, 1])),
    )
    assert r == 0
    ...
    assert np.array_equiv(
        ns,
        domain.properties2StateVec(locs, fuel - 4, np.array([1, 1]), np.array([1, 1])),
    )
