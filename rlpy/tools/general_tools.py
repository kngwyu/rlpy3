...
def count_nonzero(arr):
    ...
    *arr*, such that only individual elements are examined. \n
    ...
    # Is this an instance of a matrix? Use inbuilt nonzero() method and count
    # of indices returned.
...
def hasFunction(object_, methodname):
    """ Test if class of ``object`` has a method called ``methodname``. """
    method = getattr(object_, methodname, None)
